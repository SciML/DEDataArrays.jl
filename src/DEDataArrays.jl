...
@generated function Base.zero(A::DEDataArray)
    ...
    return :(SciMLBase.parameterless_type(A)($(assignments...)))
end
...
@generated function copy_fields(arr::AbstractArray, template::DEDataArray)
    ...
    return :(SciMLBase.parameterless_type(template)($(assignments...)))
end
...
LinearAlgebra.ldiv!(F::Factorization, A::Base.ReshapedArray{T1,T2,T3,T4}) where {T1,T2,T3<:DEDataArray,T4} = ldiv!(F,
                                                                                                                   vec(A.parent.x))
Base.:+(::LinearAlgebra.UniformScaling, x::DEDataArray) = copy_fields(I + x.x, x)

Base.unsafe_convert(::Type{Ptr{T}}, a::DEDataArray{T}) where {T} = Base.unsafe_convert(Ptr{T},
                                                                                       getfield(a,
                                                                                                :x))
...
function find_dedata(args::Tuple)
    return !isempty(args) && find_dedata(find_dedata(args[1]), Base.tail(args))
end
find_dedata(x) = x
...
@inline function Base.copy(bc::Broadcast.Broadcasted{DEDataArrayStyle})
    out = find_dedata(bc)
    return copy_fields(copy(unpack(bc)), out)
end

# drop DEData part
@inline unpack(bc::Broadcast.Broadcasted{Style}) where {Style} = Broadcast.Broadcasted{Style}(bc.f,
                                                                                              unpack_args(bc.args))
@inline unpack(bc::Broadcast.Broadcasted{DEDataArrayStyle}) = Broadcast.Broadcasted(bc.f,
                                                                                    unpack_args(bc.args))
unpack(x) = x
